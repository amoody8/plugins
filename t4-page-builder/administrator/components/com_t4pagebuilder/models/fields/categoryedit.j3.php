...
class JFormFieldCategoryEdit extends JFormFieldList
{
    ...
    protected function getOptions()
    {
        ...
        if ($this->element['parent'] || $jinput->get('option') == 'com_t4pagebuilder') {
            $oldCat = $jinput->get('id', 0);
            $oldParent = $this->form->getValue($name, 0);
            $extension = $this->element['extension'] ? (string) $this->element['extension'] : (string) $jinput->get('extension', 'com_t4pagebuilder');
        } else {
            // For items the old category is the category they are in when opened or 0 if new.
            $oldCat = $this->form->getValue($name, 0);
            $extension = $this->element['extension'] ? (string) $this->element['extension'] : (string) $jinput->get('option', 'com_t4pagebuilder');
        }
        ...
        if ($this->element['parent'] == true || $jinput->get('option') == 'com_t4pagebuilder') {
        ...
        // If parent isn't explicitly stated but we are in com_t4pagebuilder assume we want parents
        if ($oldCat != 0 && ($this->element['parent'] == true || $jinput->get('option') == 'com_t4pagebuilder')) {
