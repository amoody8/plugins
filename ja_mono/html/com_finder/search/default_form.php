...
<?php endif; ?>
		// add collapse.
		jQuery('#button-advancedSearch').on('click', function(){
			if (jQuery(jQuery(this).attr('href')).hasClass('in'))
				jQuery('.collapse').collapse('hide');
			else jQuery('.collapse').collapse('show');
		});
		jQuery('#button-advancedSearch').attr('href', '#advancedSearch'); // reset the href of button after ajax page.
	});
</script>
...
<form id="finder-search" action="<?php echo JRoute::_($this->query->toURI()); ?>" method="get" class="form-inline">
	...
	<fieldset class="word">
		<div class="form-group search-form">
			<label for="q">
				<?php echo JText::_('TPL_FINDER_SEARCH_TERMS'); ?>
			</label>
		...
		<div class="form-group">
			...
			<?php endif; ?>
			<?php if ($this->params->get('show_advanced', 1)) : ?>
				<a id="button-advancedSearch" href="#advancedSearch" data-toggle="collapse" class="btn btn-primary"><span class="fa fa-list"></span> <?php echo JText::_('COM_FINDER_ADVANCED_SEARCH_TOGGLE'); ?></a>
			<?php endif; ?>
		</div>
	</fieldset>
